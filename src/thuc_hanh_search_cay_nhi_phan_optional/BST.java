...
public class BST < E extends Comparable<E>> extends AbstractTree<E> {
    ...
    public boolean insert(E e) {
        ...
        root = createNewNode(e);
            ...
            while (current != null) {
                if (e.compareTo(current.elements) < 0) {
                    parent = current;
                    current = current.left;
                } else if (e.compareTo(current.elements) > 0) {
                    parent = current;
                    current = current.right;
                } else
                    return false;
                }
            if (e.compareTo(parent.elements) < 0)
    ...
    protected TreeNode<E> getElements(E e) {
        if (!isExits(e))
            return null;
        TreeNode<E> current = root;
        while (current != null) {
    ...
    public boolean delete(E e) {
        if (!isExits(e))
            return false;
        if (e.compareTo(root.elements)== 0) {
            root = null;
            return true;
        }
        TreeNode<E> parent = getParent(e);
        TreeNode<E> current = getElements(e);
        boolean inLeft = parent.elements.compareTo(current.elements) > 0;
        boolean inRight = parent.elements.compareTo(current.elements) < 0;
        ...
        TreeNode<E> parentOfRightMost = getParent(rightMost.elements);
